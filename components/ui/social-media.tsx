      ...
      {items.map((item) => (
        <li key={item.ariaLabel} className="group relative">
          ...
          >
            <span
              aria-hidden="true"
              className="absolute inset-x-0 bottom-0 h-0 transition-[height] duration-300 ease-in-out group-hover:h-full group-focus-within:h-full"
          ...
          <span
            role="tooltip"
            className="pointer-events-none absolute left-1/2 top-full z-10 mt-2 -translate-x-1/2 whitespace-nowrap rounded-md px-2.5 py-1.5 text-xs text-midnight-shadow opacity-0 transition-all duration-300 ease-in-out group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100 motion-safe:-translate-y-1"
            style={{ backgroundColor: item.color }}
          >
            {item.tooltip}
          </span>
