  ...
  return (
    <section className="heritage-menu content-shell" id="heritage-menu" aria-labelledby="heritage-menu-title">
      ...
      <div className="heritage-grid">
        {visibleItems.map((item) => <article className="heritage-card" key={item.name}><div className="heritage-card-image"><Image src={item.image} alt={item.name} width={500} height={500} sizes="(max-width: 700px) 42vw, 220px" /></div><div className="heritage-card-body"><h3>{item.name}</h3><p>{item.detail}</p><div className="heritage-card-footer"><strong>{item.price}</strong><a href="#contact">Add to cart <span aria-hidden="true">↗</span></a></div></div></article>)}
      </div>
